...
holidays_dataframe_transformed = holidays_dataframe.withColumn(
    "`date.iso`", f.to_timestamp(f.col("`date.iso`"))
).withColumn("type", f.regexp_extract(f.col("type"), "(\w+)", 1))

holidays_dataframe_clean = holidays_dataframe_transformed.selectExpr(
    "name",
    "description",
    "type",
    "primary_type",
...
holidays_dataframe_clean.write.format("parquet").mode("overwrite").save(
    f"s3://{BUCKET_NAME}/holidays"
)
